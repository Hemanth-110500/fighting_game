...
class Player {
  ...
  heal (player) {
    ...
    return `Player ${player.name} heals for ${hpAmount} HP `
  }
}
...
// Else if isOver is true AND p2 health is <= 0 then update message variable  to 'p2 WINS!'
  ...
  play(p1, p2) {
    ...
    while (!this.isOver) {
      ...
      p2.heal()
      p2.strike(p1, p2, p2.attackDamage)
      p1.heal()
    }
...
let player1 = new Player('Hemanth', 100, 10)
let player2 = new Player('Ram', 100, 10)
...
// ** Add a click listener to the simulate button that runs the play() method on click and pass in the players **
    playButton.onclick = () => result.innerText =
    game.play(p1, p2)
